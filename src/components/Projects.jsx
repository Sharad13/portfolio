import { FiExternalLink } from "react-icons/fi";
import { AiFillGithub,AiFillLinkedin } from "react-icons/ai";
import Aos from 'aos';
import "aos/dist/aos.css"
import { useEffect} from 'react';

export function Projects(){

    useEffect(() => {
        Aos.init({duration:1000});
      }, [])

    return<>
    <div id="projects" className=" projects container proj pb-1 ">
    <span className="headingskill">PROJECTS</span>
    <div className="row">
         <div data-aos="zoom-in" className=" col-xl-4 offset-xl-1 col-12 mt-3 mb-5 pbox pt-2 pb-3 ">
         <div className=" bord pt-3">
         <a target="_blank" href="https://sharad13.github.io/CultFit/"><img className="img-fluid p1" src="/cultfithomepage.png" alt=""/></a>
         </div>
         <span className="projname">
         <a className="linked redirect" target="_blank" href="https://sharad13.github.io/CultFit/">Cult Fit Clone  </a>
         <a className="linked1" target="_blank" href="https://github.com/Sharad13/CultFitWithNode"><AiFillGithub /></a>
         <a className="linked1" target="_blank" href="https://sharad13.github.io/CultFit/">
         <img style={{paddingLeft:"3px"}} src="https://img.icons8.com/windows/32/4a90e2/external-link.png"/>
         </a>
         </span>
         <br/>
         <span className="info">A web application which helps people to stay healthy
          and fit within the comfort of their home using live workout sessions.</span>
          <br />
          
          <div className="mt-4">
          <div style={{
            display:"grid",
            gridTemplateColumns:"1fr 1fr 1fr",
            margin:"auto",
            width:"60%",
            fontSize:"13px",
            color:"white",
            fontWeight:"700"
        }}>
        <div style={{
            width:"50px",
            marginLeft:"8px"
        }}><img className="img-fluid" src="https://img.icons8.com/color/480/000000/html-5--v1.png" alt=""/><br/>HTML</div>
        <div style={{
          width:"50px",
          marginLeft:"8px"
      }}><img className="img-fluid" src="https://img.icons8.com/color/480/000000/css3.png" alt=""/><br/>CSS</div>
      <div style={{
          width:"50px",
          marginLeft:"8px"
      }}><img className="img-fluid" src="https://img.icons8.com/color/480/000000/javascript.png" alt=""/><br/>JS</div>
      </div>
          </div>
          </div>


         <div data-aos="zoom-in" className=" col-xl-4 offset-xl-2 pbox col-12 mt-3 pt-2 mb-5">
         <div className=" bord pt-3">
         <a target="_blank" href="https://olx-clone-team4.vercel.app/"><img className="img-fluid p1" src="https://user-images.githubusercontent.com/72969348/135753076-b32dfd7d-545d-42b6-9b0b-37b8cc8f700b.png" alt=""/></a>
         </div>
         <span className="projname">
         <a className="linked redirect pr-3" target="_blank" href="https://olx-clone-team4.vercel.app/">OLX Clone</a> 
         <a className="linked1 redirect ml-3" target="_blank"  href="https://github.com/cybervirus997/Unit4-project"><AiFillGithub className=" ml-3"   /></a>
         <a className="linked1" target="_blank" href="https://olx-clone-team4.vercel.app/">
         <img style={{paddingLeft:"3px"}} src="https://img.icons8.com/windows/32/4a90e2/external-link.png"/>
         </a>
         <span>
         <a className="linked1" target="_blank" href="https://olx-clone-team4.vercel.app/">
         <img style={{width:"25px"}} src="https://img.icons8.com/pastel-glyph/64/fa314a/external-link--v1.png"/>
      </a>
      </span>
         </span>
         <br/>
         <span className="info " style={{
            color: "black",
            fontSize: "15px",
            fontWeight: "500",
            letterSpacing: "1px",
        
         }}>The OLX marketplace is a platform for buying and selling services and goods such 
         as electronics, fashion items,household goods.</span>
          <br/>
          
          <div className="mt-4">
          <div style={{
              display:"grid",
              gridTemplateColumns:"1fr 1fr 1fr 1fr",
              margin:"auto",
              width:"60%",
              fontSize:"13px",
              color:"white",
              fontWeight:"700"
          }}>
          <div style={{
              width:"50px",
              marginLeft:"8px"
          }}><img className="img-fluid" src="https://img.icons8.com/color/480/000000/react-native.png" alt=""/><br/>React</div>
          <div style={{
            width:"50px",
            marginLeft:"8px"
        }}><img className="img-fluid" src="https://img.icons8.com/color/480/000000/css3.png" alt=""/><br/>CSS</div>
        <div style={{
            width:"50px",
            marginLeft:"8px"
        }}><img className="img-fluid" src="https://img.icons8.com/color/480/000000/javascript.png" alt=""/><br/>JS</div>
        <div style={{
            width:"40px",
            marginLeft:"8px",
            marginTop:"6px"
        }}><img className="img-fluid pb-1 " src="data:image/jpeg;base64," alt=""/><br/>MongoDB</div>
        
        </div>
        </div>
         </div>
        
        
         
    </div>
</div>
<div id="projects" className="container proj pb-5 ">
    <div className="row">
         <div data-aos="zoom-in" className=" col-xl-4 offset-xl-1 col-12 mt-3 mb-5 pbox pt-2 pb-3 ">
         <div className=" bord pt-3">
         <a target="_blank" href="https://product-hunt-frontend.herokuapp.com/"><img className="img-fluid p1" src="/phunt.png" alt=""/></a>
         </div>
         <span className="projname">
         <a className="linked redirect" target="_blank" href="https://product-hunt-frontend.herokuapp.com/">Product Hunt Clone  </a>
         <a className="linked1" target="_blank" href="https://github.com/gsunil1996/p-hunt-website-clone/tree/master"><AiFillGithub /></a>
         <a className="linked1" target="_blank" href="https://product-hunt-frontend.herokuapp.com/">
            <img style={{paddingLeft:"3px"}} src="https://img.icons8.com/windows/32/4a90e2/external-link.png"/>
         </a>
         <span>
         <a className="linked1" target="_blank" href="https://youtu.be/1l8dV5hirqs">
         <img style={{width:"25px"}} src="https://img.icons8.com/pastel-glyph/64/fa314a/external-link--v1.png"/>
      </a>
      </span>
         </span>
         <br/>
         <span className="info">This web application is a platform to share and discover new products
         </span>
          <br />
          <div className="mt-4">
          <div style={{
              display:"grid",
              gridTemplateColumns:"1fr 1fr 1fr 1fr",
              margin:"auto",
              width:"60%",
              fontSize:"13px",
              color:"white",
              fontWeight:"700"
          }}>
          <div style={{
              width:"50px",
              marginLeft:"8px"
          }}><img className="img-fluid" src="https://img.icons8.com/color/480/000000/react-native.png" alt=""/><br/>React</div>
          <div style={{
            width:"50px",
            marginLeft:"8px"
        }}><img className="img-fluid" src="https://img.icons8.com/color/480/000000/css3.png" alt=""/><br/>CSS</div>
        <div style={{
            width:"50px",
            marginLeft:"8px"
        }}><img className="img-fluid" src="https://img.icons8.com/color/480/000000/javascript.png" alt=""/><br/>JS</div>
        <div style={{
            width:"40px",
            marginLeft:"8px",
            marginTop:"6px"
        }}><img className="img-fluid pb-1 " src="data:image/jpeg;base64," alt=""/><br/>MongoDB</div>
        
        </div>
        </div>
          </div>


          <div data-aos="zoom-in" className=" col-xl-4 offset-xl-2 pbox col-12 mt-3 pt-2 mb-5">
          <div className=" bord pt-3">
          <a target="_blank" href="https://drive.google.com/file/d/1dCHoDKq28ffpE4BMLsdE4QKokkDrQtQh/view"><img className="img-fluid p1" src="https://portfolio-kkrishna.vercel.app/static/media/captchaMart.19a91ab1.png" alt=""/></a>
          </div>
          <span className="projname">
          <a className="linked redirect pr-3" target="_blank" href="https://drive.google.com/file/d/1dCHoDKq28ffpE4BMLsdE4QKokkDrQtQh/view">Hackathon Project</a> 
          <a className="linked1 redirect ml-3" target="_blank"  href="https://github.com/kkm980/hackathon-CaptchaMart"><AiFillGithub className=" ml-3"   /></a>
          <a className="linked1" target="_blank" href="https://drive.google.com/file/d/1dCHoDKq28ffpE4BMLsdE4QKokkDrQtQh/view">
          <img style={{paddingLeft:"3px"}} src="https://img.icons8.com/windows/32/4a90e2/external-link.png"/>
          </a>
         
          </span>
          <br/>
          <span className="info " style={{
             color: "black",
             fontSize: "15px",
             fontWeight: "500",
             letterSpacing: "1px",
         
          }}>A logistics system where street vendors can list
           what they are offering with their live location & customers 
           can find the nearby vendors</span>
           <br/>
           
           <div className="mt-1">
           <div style={{
               display:"grid",
               gridTemplateColumns:"1fr 1fr 1fr 1fr",
               margin:"auto",
               width:"60%",
               fontSize:"13px",
               color:"white",
               fontWeight:"700"
           }}>
           <div style={{
               width:"50px",
               marginLeft:"8px"
           }}><img className="img-fluid" src="https://img.icons8.com/color/480/000000/react-native.png" alt=""/><br/>React</div>
           <div style={{
             width:"50px",
             marginLeft:"8px"
         }}><img className="img-fluid" src="https://img.icons8.com/color/480/000000/css3.png" alt=""/><br/>CSS</div>
         <div style={{
             width:"50px",
             marginLeft:"8px"
         }}><img className="img-fluid" src="https://img.icons8.com/color/480/000000/javascript.png" alt=""/><br/>JS</div>
         <div style={{
             width:"40px",
             marginLeft:"8px",
             marginTop:"6px"
         }}><img className="img-fluid pb-1 " src="data:image/jpeg;base64," alt=""/><br/>MongoDB</div>
         
         </div>
         </div>
          </div>
        
        
         
    </div>
</div>
    </>
}
// {   <span className="tools">React | CSS | JavaScript | MongoDB </span>}